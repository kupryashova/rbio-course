...
max_cor_site <- order(abs(cor(for_cor[,1], for_cor[,2:ncol(for_cor)])), decreasing = TRUE)[1:10]
...
```{r}
set.seed(255)
training <- sample(4:53, 40)
validation <- (4:53)[-training]
...
train_res <- ages$Age[ages$Sample %in% rownames(train)]
valid_res <- ages$Age[ages$Sample %in% rownames(valid)]
```
##Построение лесов
...
b <- vector(length=40)
for (i in (1:40)){
  out <- wrapper(train, train_res, valid, valid_res,runs.number = 50,sampsize = i,replace = T, ntree=50, mtry=10, nodesize=1)
a[i] <- out[2]
b[i] <- out[1]
}
...
b <- vector(length=40)
for (i in (1:40)){
  out <- wrapper(train, train_res, valid, valid_res,runs.number = 50,sampsize = i,replace = F, ntree=50, mtry=10, nodesize=1)
a[i] <- out[2]
b[i] <- out[1]
}
...
На этих легко переобучаемых деревьях переобучение наступает сразу. replace=F дает менее переобученное дерево.
...
b <- vector(length=40)
for (i in (1:40)){
  out <- wrapper(train, train_res, valid, valid_res,runs.number = 50,sampsize = 3,replace = F, ntree=50, mtry=10, nodesize=i)
a[i] <- out[2]
b[i] <- out[1]
}
nodesize_table <- data.frame(test_RMSE = a, train_RMSE = b,sampsize = 1:40)

ggplot(data = nodesize_table, aes(sampsize))+
  geom_line(aes(y = a, colour = "Validation"))+geom_line(aes(y = b, colour = "Training"))
```

Модель переобучена при малых значениях  nodesize. 
nodesize=7
...
b <- vector(length=10)
for (i in (1:10)){
  out <- wrapper(train, train_res, valid, valid_res,runs.number = 50,sampsize = 3,replace = T, ntree=50, mtry=i, nodesize=7)
a[i] <- out[2]
b[i] <- out[1]
}
mtry_table <- data.frame(test_RMSE = a, train_RMSE = b,sampsize = 1:10)

ggplot(data = mtry_table, aes(sampsize))+
  geom_line(aes(y = a, colour = "Validation"))+geom_line(aes(y = b, colour = "Training"))
```

Переобучение увеличивается при увеличении mtry. Если под переобучением подразумевать увеличение разницы между ошибкой на обучающих и валидирующих данных. Сама ошибка на оучающих данных с ростом mtry растет незначительно.
mtry=4
...
cross.results2 <- apply(cross.validation, 1, function(test.sample){
  ...
  # calculating RMSE for every part and default random forest
  return(wrapper(train.data, train.response, test.data, test.response, runs.number=100, nodesize=7, replace=F, ntree=100, sampsize=3, mtry=4))
})
...
И выясним, что ошибка на тренировочных и валидирующих данных при кроссвалидации с использованием параметров по умолчанию ниже, чем при подобранных вручную.
